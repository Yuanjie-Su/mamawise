...
function updatePartialRcordsPrompt(oldPrompt, updateType, partialRecords) {
  try {
    ...
    if (!promptLines || promptLines.length === 0) {
      promptLines = ['基于用户的健康记录：', '', '最近的体征记录：', '', '当前用药：', '', '最近产检记录：'];
    }
    ...
    switch (updateType) {
      ...
      case 'checkupRecords':
        // 更新产检记录
        promptLines = updateCheckupPrompt(promptLines, partialRecords.checkupRecords, partialRecords.checkupAnalysis);
        break;
      default:
        break;
    }
...
function updateAllergyPrompt(promptLines, allergyInfo) {
  ...
  // 生成新的过敏信息行
  const allergyLine = allergyInfo && allergyInfo.length > 0 ?
    `过敏史：${allergyInfo.join('、')}` :
    null;
...
function updateDietPrompt(promptLines, dietPreference) {
  ...
  // 生成新的饮食偏好行
  const dietLine = dietPreference && dietPreference.length > 0 ?
    `饮食偏好：${dietPreference.join('、')}` :
    null;
...
function updateOtherInfoPrompt(promptLines, otherInfo) {
  ...
  for (let i = 0; i < promptLines.length; i++) {
    if (promptLines[i].includes('年龄：')) {
      ...
      heightLineIndex = i;
    } else if (promptLines[i].includes('体重：') && !promptLines[i].includes('kg (')) { // 避免匹配到体征记录中的体重
      weightLineIndex = i;
    }
  }
...
function updateWeightPrompt(promptLines, weightRecords) {
  ...
  for (let i = 0; i < promptLines.length; i++) {
    if (promptLines[i].includes('体重：') && promptLines[i].includes('kg (')) { // 确保匹配的是体征记录中的体重
      weightLineIndex = i;
      break;
    }
  }
...
function updateMedicationsPrompt(promptLines, medications) {
  ...
  // 查找用药记录部分的结束位置
  let medSectionEnd = promptLines.findIndex((line, index) =>
    index > medSectionStart && line.includes('：') && !line.match(/^药物\d+：/)
  );
  ...
  if (medications && medications.length > 0) {
    medications.forEach((med, index) => {
      const medLine = `药物${index + 1}：${med.name}，剂量：${med.dosage}，频率：${med.frequency}` +
...
function updateCheckupPrompt(promptLines, checkupRecords, checkupAnalysis) {
  ...
  // 查找产检记录部分的结束位置
  let checkupSectionEnd = promptLines.findIndex((line, index) =>
  ...
  if (checkupRecords && checkupRecords.length > 0) {
    ...
    recentCheckups.forEach((record, index) => {
      const checkupLine = `产检${index + 1}：${record.date}（孕${record.week}周），医院：${record.hospital}，` +
...
function findBasicInfoSection(promptLines) {
  ...
  let end = promptLines.findIndex((line, index) =>
    index > 0 && line.includes('：') && !line.match(/^(当前孕周|预产期|过敏史|饮食偏好|年龄|身高|体重)：/)
  );
