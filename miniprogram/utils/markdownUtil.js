  ...
  try {
    ...
    const formatRules = [
      ...
      // 处理代码块
      {
        regex: /```([\s\S]*?)```/g,
        replacement:
          '<div style="background-color:#f5f5f5;padding:8px;border-radius:4px;font-family:monospace;white-space:pre-wrap;margin:8px 0;font-size:12px;">$1</div>',
      },
      // 处理行内代码
      {
        regex: /`([^`]+)`/g,
        replacement:
          '<span style="background-color:#f5f5f5;padding:2px 4px;border-radius:3px;font-family:monospace;font-size:12px;">$1</span>',
      },
      // 处理水平线
      {
        regex: /^---+$/gm,
...
/**
 * 格式化 Markdown 文本为 HTML，但排除最后一行的"---标题"格式
 * @param {String} text - 原始 Markdown 文本
 * @returns {String} 格式化后的 HTML 文本
 */
function formatMarkdownExcludingLastLine(text) {
  if (!text) return ''
  ...
  try {
    ...
    if (lines.length > 0) {
      ...
      if (lastLine.startsWith('---')) {
        // 排除最后一行
        contentToFormat = lines.slice(0, lines.length - 1).join('\n')
      }
    }

    // 使用现有的formatMarkdown函数处理内容
    return formatMarkdown(contentToFormat)
  } catch (error) {
    Logger.error('排除最后一行的Markdown格式化错误:', error)
    // 如果格式化失败，返回纯文本
    return text.replace(/\n/g, '<br>')
  }
}
...
export default {
  formatMarkdown,
  formatMarkdownExcludingLastLine,
  containsMarkdown,
  stripMarkdown,
}
